...
LongNovelInsight is a local tool. You provide your own LLM API key (DeepSeek or any
OpenAI-compatible provider). The key is stored in the local SQLite database, omitted from API
responses, and sent only to the provider endpoint you select when you explicitly test a provider,
run analysis, or use evidence-backed chat.
...
Configure the provider base URL, model, and API key in the Providers page before running a
connection test, analysis, or chat. Product runtime does not read `DEEPSEEK_API_KEY`; that
environment variable is used only by opt-in historical smoke utilities.

## Documentation Map

| Concern | Document |
| --- | --- |
| Current product boundary | [Product specification](docs/SPEC.md) |
| Components, runtime, and storage | [Architecture](docs/ARCHITECTURE.md) |
| REST endpoint map | [API guide](docs/API.md) |
| Analysis lifecycle and legacy boundary | [AnalysisRun contract](docs/ANALYSIS_RUN_CONTRACT.md) |
| LLM calls, prompts, retries, and usage | [LLM pipeline](docs/LLM_PIPELINE.md) |
| SQLite records and relationships | [Data model](docs/DATA_MODEL.md) |
| Release direction | [Roadmap](docs/ROADMAP.md) |
| Current release draft | [v0.4.0 release notes](docs/releases/v0.4.0.md) |
| Development and handoff process | [Development workflow](docs/DEV_WORKFLOW.md) |
| Last verified repository state | [Project status](agent/PROJECT_STATUS.md) |

Exact live request/response schemas are generated by FastAPI at
`http://127.0.0.1:8000/docs` and `http://127.0.0.1:8000/openapi.json`.
...
The default-safe v0.4 Work smoke is `backend/tests/test_v04_integrated_smoke.py` and uses temporary
storage plus a mocked LLM extraction boundary. Historical live/backend smoke scripts and the v0.3
opt-in integration suite remain available. See [docs/SMOKE_TEST.md](docs/SMOKE_TEST.md) for details.
