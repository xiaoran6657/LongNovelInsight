...
def _setup_entities_and_relations(engine):
    ...
    with Session(engine) as session:
        prov = ModelProvider(
            name="GraphP", provider_type="openai_compatible",
        ...
        char_a = GlobalEntity(
            topic_id=topic.id, entity_type="character",
            canonical_name="Alice", aliases_json='["A"]',
            work_ids_json=json.dumps([wids[0]]), mention_count=1,
            confidence=0.92, merge_strategy="exact",
        )
        char_b = GlobalEntity(
            topic_id=topic.id, entity_type="character",
            canonical_name="Bob", aliases_json='["B"]',
            work_ids_json=json.dumps([wids[1]]), mention_count=1,
            confidence=0.92, merge_strategy="exact",
        )
        session.add(char_a); session.add(char_b); session.flush()

        # Entity mentions with stable_id for graph resolution
        session.add(EntityMention(
            topic_id=topic.id, global_entity_id=char_a.id, work_id=wids[0],
            source_type="atom", source_id="a1", chunk_id=cids[0],
            surface_text="Alice",
            metadata_json=json.dumps({"stable_id": "char_alice"}),
        ))
        session.add(EntityMention(
            topic_id=topic.id, global_entity_id=char_b.id, work_id=wids[1],
            source_type="atom", source_id="a2", chunk_id=cids[1],
            surface_text="Bob",
        ...
        session.add(ExtractedAtom(
            run_id=run.id, topic_id=topic.id, chunk_id=cids[0],
            atom_type=AtomType.RELATION, stable_id="rel_ab",
            content_json=json.dumps({
                "character_a": "Alice", "character_b": "Bob",
                "relation_type": "ally", "evidence": "Allies",
            }),
            source_chunk_ids=json.dumps([cids[0]]),
            evidence_quotes=json.dumps(["Allies"]), confidence=0.85,
        ))
        session.commit()
...
class TestGraphBuild:
    ...
    def test_graph_filters_work_id(self, engine, client):
        ...
        r = client.get(
            f"/api/topics/{tid}/graphs/characters?work_id={wids[0]}"
        )
        assert r.status_code == 200
        data = r.json()
    ...
    def test_graph_filters_min_confidence(self, engine, client):
        ...
        r = client.get(
            f"/api/topics/{tid}/graphs/characters?min_confidence=0.9"
        )
        assert r.status_code == 200
    ...
    def test_event_cooccurrence_fallback(self, engine):
        ...
        with Session(engine) as session:
            prov = ModelProvider(
                name="CoocP", provider_type="openai_compatible",
                base_url="http://mock", api_key="sk-m", model_name="m", is_default=True,
            )
            session.add(prov); session.flush()
            ...
            session.add(w); session.flush()
            d = Document(topic_id=topic.id, work_id=w.id, original_filename="w.txt",
            ...
            char_a = GlobalEntity(
                topic_id=topic.id, entity_type="character",
                canonical_name="Alice", work_ids_json=json.dumps([w.id]),
                mention_count=1, confidence=0.92,
            )
            char_b = GlobalEntity(
                topic_id=topic.id, entity_type="character",
                canonical_name="Bob", work_ids_json=json.dumps([w.id]),
                mention_count=1, confidence=0.92,
            )
            session.add(char_a); session.add(char_b); session.flush()
            session.add(EntityMention(
                topic_id=topic.id, global_entity_id=char_a.id, work_id=w.id,
                source_type="atom", source_id="a1", chunk_id=ck.id,
                surface_text="Alice",
                metadata_json=json.dumps({"stable_id": "char_a"}),
            ))
            session.add(EntityMention(
                topic_id=topic.id, global_entity_id=char_b.id, work_id=w.id,
                source_type="atom", source_id="a2", chunk_id=ck.id,
                surface_text="Bob",
            ...
            session.add(ExtractedAtom(
                run_id=run.id, topic_id=topic.id, chunk_id=ck.id,
                atom_type=AtomType.EVENT, stable_id="evt_ab",
                content_json=json.dumps({
                    "title": "Meeting", "participants": ["Alice", "Bob"],
                }),
