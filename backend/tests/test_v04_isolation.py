...
class TestSourceFileIsolation:
    def test_two_works_upload_same_format_no_overwrite(self, engine, client):
        ...
        r1 = client.post(
            ...
            files={
                "file": ("w1.txt", io.BytesIO("Work One\n第一章 内容。\n".encode()), "text/plain")
            },
        )
        ...
        r2 = client.post(
            ...
            files={
                "file": ("w2.txt", io.BytesIO("Work Two\n第一章 不同。\n".encode()), "text/plain")
            },
        )
...
class TestDeleteIsolation:
    def test_legacy_delete_does_not_affect_other_works(self, engine, client):
        ...
        assert len(w2_chunks_after) == w2_chunk_count, (
            f"Work 2 chunks should survive legacy delete: {w2_chunk_count} → {len(w2_chunks_after)}"
        )
...
class TestEntityRebuildClears:
    def test_empty_rebuild_clears_old_registry(self, engine):
        ...
        with Session(engine) as session:
            ...
            entities = session.exec(select(GlobalEntity).where(GlobalEntity.topic_id == tid)).all()
            assert len(entities) == 1

            # Delete all atoms, rebuild with empty
            session.exec(
                __import__("sqlmodel").delete(ExtractedAtom).where(ExtractedAtom.topic_id == tid)
            )
            session.commit()
            result2 = build_entity_registry(tid, session)
            assert result2["entity_count"] == 0
            entities2 = session.exec(select(GlobalEntity).where(GlobalEntity.topic_id == tid)).all()
...
class TestDeleteCleansOwnAnalysis:
    def test_delete_removes_only_own_extractions_and_atoms(self, engine, client):
        ...
        with Session(engine) as session:
            w1_doc = session.exec(select(Document).where(Document.work_id == w1_id)).first()
            w2_doc = session.exec(select(Document).where(Document.work_id == w2_id)).first()
