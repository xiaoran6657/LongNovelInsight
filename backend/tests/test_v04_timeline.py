...
def _setup_timeline_data(engine):
    ...
    with Session(engine) as session:
        prov = ModelProvider(
            name="TimelineP", provider_type="openai_compatible",
        ...
        ch0 = Chapter(
            ...
            chapter_index=0, title="Ch0",
            start_char=0, end_char=50, char_count=50,
        )
        ch1 = Chapter(
            topic_id=topic.id, document_id=d.id,
            chapter_index=1, title="Ch1",
            start_char=50, end_char=100, char_count=50,
        )
        session.add(ch0); session.add(ch1); session.flush()
        ck0 = Chunk(
            topic_id=topic.id, document_id=d.id, chapter_id=ch0.id,
            chapter_index=0, chunk_index=0, text="Ch0 text",
            start_char=0, end_char=50, char_count=50, estimated_tokens=34,
        )
        ck1 = Chunk(
            topic_id=topic.id, document_id=d.id, chapter_id=ch1.id,
            chapter_index=1, chunk_index=0, text="Ch1 text",
            start_char=50, end_char=100, char_count=50, estimated_tokens=34,
        )
        session.add(ck0); session.add(ck1); session.flush()

        # Event atoms
        session.add(ExtractedAtom(
            run_id=run.id, topic_id=topic.id, chunk_id=ck0.id,
            atom_type=AtomType.EVENT, stable_id="evt_later",
            title="Later Event",
            content_json=json.dumps({
                "title": "Later Event", "participants": ["Alice"],
            }),
            source_chunk_ids=json.dumps([ck0.id]),
            evidence_quotes=json.dumps(["later"]), confidence=0.9,
        ))
        session.add(ExtractedAtom(
            run_id=run.id, topic_id=topic.id, chunk_id=ck1.id,
            atom_type=AtomType.EVENT, stable_id="evt_earlier",
            title="Earlier Event",
            content_json=json.dumps({
                "title": "Earlier Event", "participants": ["Bob"],
            }),
            source_chunk_ids=json.dumps([ck1.id]),
            evidence_quotes=json.dumps(["earlier"]), confidence=0.9,
        ))
        session.commit()
...
class TestTimelineBuild:
    ...
    def test_timeline_items_persisted(self, engine):
        ...
        with Session(engine) as session:
            ...
            items = session.exec(
                select(TimelineItem).where(TimelineItem.topic_id == tid)
            ).all()
            assert len(items) == 2
